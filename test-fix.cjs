...
async function main() {
  // Load the fixed WASM decoder (CJS build)
  const decoder = require('./packages/2.5.4-decoder/dist/index.js').default;
  ...
  const tmpPng = path.join(os.tmpdir(), `test_fix_${Date.now()}.png`);
  ...
  try {
    ...
    if (diffCount < minLen * 0.01) {
      console.log(`\n  PASS: WASM and native decoders MATCH!`);
    } else {
      console.log(`\n  FAIL: WASM and native decoders DIFFER`);
    }
  } catch (err) {
